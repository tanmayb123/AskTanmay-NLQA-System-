...
class CAFS {
    ...
    func runNERFNS() -> [(String, Int)] {
        ...
        for i in candidateAnswers[0...zeroTo] {
            var alnum: NSMutableCharacterSet = NSMutableCharacterSet(charactersInString: " ")
            alnum.formUnionWithCharacterSet(NSCharacterSet.alphanumericCharacterSet())
            let strippedReplacement: String = i.0.componentsSeparatedByCharactersInSet(alnum.invertedSet).joinWithSeparator("")
        ...
        var resultSplit = resfinal.componentsSeparatedByString("\n")
        resultSplit.removeLast()
        for (ind, val) in resultSplit.enumerate() where ind % 2 == 0 {
    ...
    func runDCM() -> [(String, Int)] {
        ...
        for i in candidateAnswers {
            var push = true
            var updateInd = 0
            for (ind, val) in finalValue.enumerate() where val.0 == i.0 {
                push = false
                updateInd = ind
            }
            if push {
    ...
    func runQAR() -> [(String, Int)] {
        var finalValue = [(String, Int)]()
        for i in candidateAnswers {
            if !(userQuery.containsString(i.0.lowercaseString)) {
                finalValue.append(i)
            }
        }
        return finalValue
